...
void Camera::setInitialState(const glm::vec3& position, const glm::vec3& direction) {
...
void Camera::update(float ts) {
    GLFWwindow* window = wen::g_window->getWindow();
    ...
    float speed = 3.0f;
    if (glfwGetKey(window, WEN_KEY_W)) {
        data.position += direction * speed * ts;
    } else if (glfwGetKey(window, WEN_KEY_S)) {
        data.position -= direction * speed * ts;
    }
    if (glfwGetKey(window, WEN_KEY_A)) {
        data.position -= right_direction * speed * ts;
    } else if (glfwGetKey(window, WEN_KEY_D)) {
        data.position += right_direction * speed * ts;
    }
    if (glfwGetKey(window, WEN_KEY_Q)) {
        data.position += up_direction * speed * ts;
    } else if (glfwGetKey(window, WEN_KEY_E)) {
        data.position -= up_direction * speed * ts;
    }
    ...
    if (delta.x != 0.0f || delta.y != 0.0f) {
        ...
        glm::quat q = glm::normalize(glm::cross(glm::angleAxis(-pitch, right_direction),
