...
layout(binding = 0, input_attachment_index = 0) uniform subpassInput position_buffer;
...
void main() {
    ...
    vec3 pos = subpassLoad(position_buffer).xyz;

    vec2 uv_per_pixel = 1 / pc.window_size;
    vec2 uv = gl_FragCoord.xy * uv_per_pixel;

    float left = float(-pc.blur_kernel_size) / 2.0;

    float ssao_value = 0;
    int count = 0;
    for (int x = 0; x < pc.blur_kernel_size; x++) {
        for (int y = 0; y < pc.blur_kernel_size; y++) {
