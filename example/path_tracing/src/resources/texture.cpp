...
glm::vec3 SolidColor::value(float u, float v, const glm::vec3& hit_point) const {
    return color_;
}
...
glm::vec3 ChessboardTexture::value(float u, float v, const glm::vec3& hit_point) const {
    int x = int(std::floor(1.0f / scale_ * hit_point.x));
    int y = int(std::floor(1.0f / scale_ * hit_point.y));
    int z = int(std::floor(1.0f / scale_ * hit_point.z));

    bool even = (x + y + z) % 2 == 0;
    return even ? even_->value(u, v, hit_point) : odd_->value(u, v, hit_point);
}
...
glm::vec3 ImageTexture::value(float u, float v, const glm::vec3& hit_point) const {
    if (height_ <= 0) {
...
glm::vec3 NoiseTexture::value(float u, float v, const glm::vec3& hit_point) const {
    auto s = scale_ * hit_point;
