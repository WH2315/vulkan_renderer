...
void RandomSpheres(Scene& scene) {
    ...
    auto ground = std::make_shared<Lambertian>(std::make_shared<ChessboardTexture>(
        0.32f, glm::vec3(0.2f, 0.3f, 0.1f), glm::vec3(0.9f, 0.9f, 0.9f)));
    world->add(
        std::make_shared<Sphere>(glm::vec3(0.0f, -1000.0f, 0.0f), 1000.0f, ground));
    ...
    auto texture = std::make_shared<ChessboardTexture>(
        0.2f, glm::vec3(0.2f, 0.3f, 0.1f), glm::vec3(0.9f, 0.9f, 0.9f));
    material = std::make_shared<Lambertian>(texture);
    world->add(std::make_shared<Sphere>(glm::vec3(10.0f, 1.0f, 0.0f), 1.0f, material));

    auto earth = std::make_shared<ImageTexture>("resources/textures/earth.jpg");
    ...
    for (int a = -n; a < n; a++) {
        for (int b = -n; b < n; b++) {
            ...
            glm::vec3 center(a + 0.9f * Random::Float(), 0.2f,
                             b + 0.9f * Random::Float());
                ...
                if (type < 0.8f) {
                    if (type < 0.6f) {
                        ...
                        world->add(std::make_shared<Sphere>(
                            center,
                            center + glm::vec3(0.0f, Random::Float(0.0f, 0.5f), 0.0f),
                            0.2f, material));
                    } else {
                        material = std::make_shared<Lambertian>(Random::Vec3());
    ...
    scene.world = std::move(
        std::make_shared<HittableList>(std::move(std::make_shared<BVH>(world))));
}
...
void CornellBox(Scene& scene) {
    ...
    world->add(std::make_shared<Quad>(glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 555.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 555.0f), green));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 555.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 555.0f), red));
    world->add(std::make_shared<Quad>(glm::vec3(113.0f, 554.0f, 127.0f),
                                      glm::vec3(330.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 305.0f), light));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 555.0f, 0.0f),
                                      glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 555.0f), white));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 0.0f, 0.0f),
                                      glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 555.0f), white));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 0.0f, 555.0f),
                                      glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 555.0f, 0.0f), white));

    std::shared_ptr<Hittable> box1 =
        box(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(165.0f, 330.0f, 165.0f), white);
    ...
    std::shared_ptr<Hittable> box2 =
        box(glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(165.0f, 165.0f, 165.0f), white);
...
void FinalScene(Scene& scene) {
    ...
    world->add(std::make_shared<Quad>(
        glm::vec3(123.0f, 554.0f, 147.0f), glm::vec3(300.0f, 0.0f, 0.0f),
    ...
    material = std::make_shared<Dielectric>(1.5);
    world->add(
        std::make_shared<Sphere>(glm::vec3(260.0f, 150.0f, 45.0f), 50.0f, material));
    ...
    world->add(
        std::make_shared<Sphere>(glm::vec3(0.0f, 150.0f, 145.0f), 50.0f, material));

    // boundary
    material = std::make_shared<Dielectric>(1.5);
    auto boundary1 =
        std::make_shared<Sphere>(glm::vec3(360.0f, 150.0f, 145.0f), 70.0f, material);
    world->add(boundary1);
    world->add(
        std::make_shared<ConstantMedium>(boundary1, 0.2f, glm::vec3(0.2f, 0.4f, 0.9f)));
    auto boundary2 =
        std::make_shared<Sphere>(glm::vec3(0.0f, 0.0f, 0.0f), 5000.0f, material);
    world->add(std::make_shared<ConstantMedium>(boundary2, 0.0001f,
                                                glm::vec3(1.0f, 1.0f, 1.0f)));

    // texture
    auto earth_texture = std::make_shared<ImageTexture>("resources/textures/earth.jpg");
    auto earth_surface = std::make_shared<Lambertian>(earth_texture);
    world->add(std::make_shared<Sphere>(glm::vec3(400.0f, 200.0f, 400.0f), 100.0f,
                                        earth_surface));
    auto perlin_texture = std::make_shared<NoiseTexture>(0.1f);
    auto perlin_surface = std::make_shared<Lambertian>(perlin_texture);
    world->add(std::make_shared<Sphere>(glm::vec3(220.0f, 280.0f, 300.0f), 80.0f,
                                        perlin_surface));
    ...
    world->add(std::make_shared<Translate>(
        std::make_shared<Rotate>(std::make_shared<BVH>(boxes), 15.0f),
...
void Life(Scene& scene) {
    ...
    world->add(std::make_shared<Quad>(glm::vec3(555.0f, 0.0f, 0.0f),
                                      ...
                                      glm::vec3(0.0f, 555.0f, 0.0f), green));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 0.0f, 555.0f),
                                      glm::vec3(0.0f, 0.0f, -555.0f),
                                      glm::vec3(0.0f, 555.0f, 0.0f), red));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 555.0f, 0.0f),
                                      glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 555.0f), white));
    world->add(std::make_shared<Quad>(glm::vec3(0.0f, 0.0f, 555.0f),
                                      glm::vec3(555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, -555.0f), white));
    world->add(std::make_shared<Quad>(glm::vec3(555.0f, 0.0f, 555.0f),
                                      glm::vec3(-555.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 555.0f, 0.0f), white));

    // Light
    world->add(std::make_shared<Quad>(glm::vec3(213.0f, 554.0f, 227.0f),
                                      glm::vec3(130.0f, 0.0f, 0.0f),
                                      glm::vec3(0.0f, 0.0f, 105.0f), light));
    ...
    // Glass sphere
    world->add(std::make_shared<Sphere>(glm::vec3(190.0f, 90.0f, 190.0f), 90.0f,
                                        std::make_shared<Dielectric>(1.5f)));

    // Light Sources
    auto m = std::shared_ptr<Material>();
    lights->add(std::make_shared<Quad>(glm::vec3(343.0f, 554.0f, 332.0f),
                                       glm::vec3(-130.0f, 0.0f, 0.0f),
                                       glm::vec3(0.0f, 0.0f, -105.0f), m));
...
RayTracing::RayTracing() : camera_(45.0f, 0.1f, 100.0f) {
    switch (4) {
        case 1: {
            ...
            camera_.setup(glm::vec3(13.0f, 2.0f, 3.0f), direction);
        ...
        case 2: {
            ...
            camera_.setup(glm::vec3(278.0f, 278.0f, -800.0f), direction);
            break;
        }
        case 3: {
            FinalScene(scene_);
            auto direction = glm::normalize(glm::vec3(-200.0f, 0.0f, 600.0f));
            camera_.setup(glm::vec3(478.0f, 278.0f, -600.0f), direction);
            break;
        }
        case 4: {
            Life(scene_);
            auto direction = glm::normalize(glm::vec3(0.0f, 0.0f, 800.0f));
            camera_.setup(glm::vec3(278, 278, -800), direction);
            break;
        }
    }
}
...
void RayTracing::render() {
    ...
    if (ImGui::Button("save image")) {
        ...
        auto filename = "resources/images/" + std::string(filename_);
    ...
    if (image) {
        ...
        ImGui::Image(image->id(), {w, h}, ImVec2(0, 1), ImVec2(1, 0));
    }
    ImGui::End();
    ImGui::PopStyleVar();
}