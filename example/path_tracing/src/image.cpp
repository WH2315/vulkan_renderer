...
void Image::create() {
    ...
    image_ = device.createImage(image_ci);

    vk::MemoryRequirements req = device.getImageMemoryRequirements(image_);
    vk::MemoryAllocateInfo ai = {};
    ai.setAllocationSize(req.size).setMemoryTypeIndex(Utils::getMemoryType(
        vk::MemoryPropertyFlagBits::eDeviceLocal, req.memoryTypeBits));
    image_memory_ = device.allocateMemory(ai);
    device.bindImageMemory(image_, image_memory_, 0);

    // create image view
    vk::ImageViewCreateInfo image_view_ci = {};
    image_view_ci.setImage(image_)
        .setViewType(vk::ImageViewType::e2D)
        .setFormat(format)
        .setSubresourceRange({vk::ImageAspectFlagBits::eColor, 0, 1, 0, 1});
    image_view_ = device.createImageView(image_view_ci);
    ...
    sampler_ = device.createSampler(sampler_ci);
...
void Image::set(const void* data) {
    ...
    if (!staging_) {
        ...
        ci.setSize(size)
            ...
            .setSharingMode(vk::SharingMode::eExclusive);
        staging_ = device.createBuffer(ci);

        vk::MemoryRequirements req = device.getBufferMemoryRequirements(staging_);
        vk::MemoryAllocateInfo ai = {};
        ai.setAllocationSize(req.size).setMemoryTypeIndex(
            Utils::getMemoryType(vk::MemoryPropertyFlagBits::eHostVisible |
                                     vk::MemoryPropertyFlagBits::eHostCoherent,
                                 req.memoryTypeBits));
        staging_memory_ = device.allocateMemory(ai);
        device.bindBufferMemory(staging_, staging_memory_, 0);

        // create buffer
        ci.setSize(size)
            .setUsage(vk::BufferUsageFlagBits::eTransferDst)
            .setSharingMode(vk::SharingMode::eExclusive);
        buffer_ = device.createBuffer(ci);

        req = device.getBufferMemoryRequirements(buffer_);
        ai.setAllocationSize(req.size).setMemoryTypeIndex(Utils::getMemoryType(
            vk::MemoryPropertyFlagBits::eDeviceLocal, req.memoryTypeBits));
        buffer_memory_ = device.allocateMemory(ai);
        device.bindBufferMemory(buffer_, buffer_memory_, 0);
    }

    // upload buffer
    void* map = device.mapMemory(staging_memory_, 0, size);
    memcpy(map, data, size);
    auto cmdbuf = Application::allocateSingleUse();
    vk::BufferCopy regions = {};
    regions.setSrcOffset(0).setDstOffset(0).setSize(size);
    cmdbuf.copyBuffer(staging_, buffer_, regions);
    Application::freeSingleUse(cmdbuf);
    device.unmapMemory(staging_memory_);
    ...
    cmdbuf.copyBufferToImage(staging_, image_, vk::ImageLayout::eTransferDstOptimal, 1,
                             &region);
...
void Image::reset() {
    Application::submitResourceFree(
        [sampler = sampler_, image_view = image_view_, image = image_,
         image_memory = image_memory_, buffer = buffer_, staging = staging_,
         buffer_memory = buffer_memory_, staging_memory = staging_memory_]() {
            auto device = Application::getDevice();
