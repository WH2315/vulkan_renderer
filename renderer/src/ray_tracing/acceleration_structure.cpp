...
void AccelerationStructure::build(bool is_update, bool allow_update) {
    ...
    for (auto& model : models_) {
        if (!is_update) {
            uint64_t vertices_size = model->vertex_count * sizeof(RayTracingVertex);
        ...
        as_info.geometries
            ...
            // 顶点数据的偏移
            .setVertexStride(sizeof(RayTracingVertex))
            // 索引数据 : uint32_t
    ...
    if (!is_update) {
        for (auto& model : models_) {
            ...
            uint64_t vertices_size = model->vertex_count * sizeof(RayTracingVertex);
            std::vector<RayTracingVertex> rt_vertices;
            rt_vertices.reserve(model->vertex_count);
            for (const auto& v : model->vertices()) {
                rt_vertices.push_back(
                    {v.position, 0.0f, v.normal, 0.0f, v.color, 0.0f});
            }
