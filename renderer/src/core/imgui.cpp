...
Imgui::Imgui(Renderer& renderer, bool docking) : renderer_(renderer) {
    IMGUI_CHECKVERSION();
    ImGui::CreateContext();
    ...
    if (docking) {
        subpass.setInputAttachment(IMGUI_DOCKING_ATTACHMENT);
    }
    ...
    init_info.PipelineInfoMain.MSAASamples = static_cast<VkSampleCountFlagBits>(renderer_config->getSampleCount());
