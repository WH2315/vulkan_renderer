...
Camera::Camera(GLFWwindow* window, float fov, float near, float far)
    : window_(window), fov_(fov), near_(near), far_(far) {}
...
bool Camera::update(float ts) {
    ...
    int space_state = glfwGetKey(window_, GLFW_KEY_SPACE);
    ...
    if (glfwGetKey(window_, GLFW_KEY_A)) {
        ...
        moved = true;
    } else if (glfwGetKey(window_, GLFW_KEY_D)) {
    ...
    if (glfwGetKey(window_, GLFW_KEY_Q)) {
        ...
        moved = true;
    } else if (glfwGetKey(window_, GLFW_KEY_E)) {
